...
class TabQPolicy(QPolicy):
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        next_ = self.discretize(next_state)
        loss = self.model[curr][action]
        if done and next_state[0] > 0.5:
            target = reward
        else:
            target = reward + self.gamma * max(self.model[next_])
        lr_decay = min(self.lr, (100 / (100 + self.N[curr][action])))
        loss += lr_decay * (target - loss)
        self.model[curr][action] = loss
        self.N[curr][action] += 1
        res = loss ** 2
        return res
